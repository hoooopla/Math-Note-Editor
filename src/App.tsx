...
export default function App() {
    ...
    const { addBlock, setActiveBlock, initBackend, connectLocalFS, openTabs, activeTab, setOpenTabs, setActiveTab } = useStore();
            ...
            <div className="w-full md:w-64 bg-surface border-r border-outline flex flex-col h-[30vh] md:h-full shrink-0">
                ...
                {backendMode === "none" ? (
                    <div className="p-4 flex flex-col items-center justify-center flex-1 text-center">
                        <FolderOpen size={32} className="text-secondary mb-3 opacity-50" />
                        <p className="text-sm text-secondary mb-4">Connect a local folder to start taking notes offline directly in your browser.</p>
                        <button 
                            onClick={() => connectLocalFS()}
                            className="px-4 py-2 bg-accent/20 text-accent rounded-lg font-medium text-sm hover:bg-accent/30 transition-colors"
                        >
